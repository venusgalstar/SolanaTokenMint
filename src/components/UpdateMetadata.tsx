...
export const UpdateMetadata: FC = () => {
  ...
  const onClickMint = useCallback(async (splTokenAddress) => {

    console.log("splTokenAddress", splTokenAddress);

    const tokenPublicKey = new PublicKey(splTokenAddress);
    ...
    await sendTransaction(setAuthorityTransaction, connection);
  }, [publicKey, connection, sendTransaction]);
  ...
  return (
    <div className="my-6">
      ...
      <button
        className="px-8 m-2 btn animate-pulse bg-gradient-to-r from-[#9945FF] to-[#14F195] hover:from-pink-500 hover:to-yellow-500 ..."
        onClick={() =>
          onClickMint(tokenAddress)
        }
      >
        <span>Update Mint Authority Free</span>
      </button>
    </div>
